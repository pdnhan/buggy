...
function makeRequest() {
  ...
  formData.set("file", new File(["title\nLogin works"], "cases.csv", { type: "text/csv" }));
  formData.set("projectId", "p1");
  return new Request("http://localhost/api/test-cases/import", {
    method: "POST",
    body: formData,
  });
}

describe("POST /api/test-cases/import", () => {
  ...
  it("returns 403 for a VIEWER (closes the VIEWER-write escalation), before parsing the file", async () => {
